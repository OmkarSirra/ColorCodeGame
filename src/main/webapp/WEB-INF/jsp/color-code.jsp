...
<script type="text/javascript">
	...
	function check(guess) {
		...
		var findMeColor = window.getComputedStyle(findMe).getPropertyValue(
				'background-color');
		var guessColor = window.getComputedStyle(guess).getPropertyValue(
				'background-color');
		//alert(findMeColor);
		//alert(guessColor);
		//alert(findMe.isEqualNode(guess));
		if ((findMeHTML.trim() === guessHTML.trim())
				&& (findMeColor === guessColor)) {
		...
		sessionStorage.setItem('clickText', document
				.getElementById('clickText').innerHTML);
	...
	function updateScore() {
		...
		if (sessionStorage.getItem('score'))
			document.getElementById('score').innerHTML = sessionStorage
					.getItem('score');
		if (sessionStorage.getItem('clickText'))
			document.getElementById('clickText').innerHTML = sessionStorage
					.getItem('clickText');
		//	alert(sessionStorage.getItem('levelChangeScore') );
	}
...
<body onload="updateScore()">
	<%
		...
		String color;
		int allowedColors = Integer.parseInt((String) request.getAttribute("level"));
	...
	<center>
		...
		<div id="upperDiv">
			<span style="float: left"> <a href="/color-code/start">
					Change level</a>
		...
		<div style="margin: 1% 40% 0% 30%">
			...
			<div id="firstDiv">
				...
				<div style="margin:"0% 40% 0% 40%; padding:"2% 5% 0% 5%">
					...
					<span>Find me
						<button id="findme"
							style="background-color:<%=colors[findeMeColor]%>">
							<%
								out.print(findMeChar);
							%>
						</button>

					</span> <span id="timerDiv" class="blue" style="float: right">
						Timer
						<button id="timer">10</button>
					</span>
				</div>
				<br> <br>
				<table border="1">
					...
					%>
					<td><button style="background-color:<%=color%>"
							onclick="check(this)">
							<%
								out.print(c);
							%>
